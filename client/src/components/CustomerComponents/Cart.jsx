...
const Cart = ({
  ...
  const handleProceedToCheckout = () => {
    ...
    navigate(`/checkout?customerId=${encodedCustomerId}&identifier=cart`, {
      state: { cartItems, orderMethod: "cart" },
    });
  };
          ...
          {cartItems.length === 0 ? (
            ...
            <div className="divide-y divide-gray-100">
                      ...
                      <div className="flex items-start justify-between">
                        <div className="pr-2">
                          ...
                          <p className="text-sm text-gray-500 mt-1">
                            {`₹${parseFloat(item.price).toFixed(2)} per ${item.unit || "item"}`}
                          </p>
                          {!isOutOfStock && (
                            <div
                              className={`text-xs mt-1 font-medium ${
                                isLowStock ? "text-amber-600" : "text-gray-500"
                              }`}
                            >
                        ...
                        <button
                          ...
                          className="p-1.5 rounded-md text-gray-400 hover:text-red-500 hover:bg-red-50 transition-colors duration-200 ml-2 mt-0.5 cursor-pointer"
                        >
                          <X size={18} />
                        </button>
                      </div>
                      ...
                      <div className="flex items-center justify-between mt-4">
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className={`w-8 h-8 rounded-md bg-gray-50 flex items-center justify-center text-gray-700 hover:bg-gray-100 transition-all duration-200 ${
                              item.quantity <= 1
                          ...
                          <span className="mx-1 w-6 text-center text-sm font-medium text-gray-800">
                            {item.quantity}
                          </span>
                          <button
                            onClick={() => updateQuantity(item.product_id, 1)}
              ...
              <div className="sticky bottom-0 bg-white border-t border-gray-100 pt-5 pb-6 px-6 shadow-[0_-4px_12px_rgba(0,0,0,0.05)]">
                ...
                <button
                  type="button"
                  className="w-full bg-green-600 hover:bg-green-700 text-white font-medium py-3 rounded-lg transition-all duration-200 text-base shadow-md hover:shadow-lg cursor-pointer"
                  onClick={handleProceedToCheckout}
                >
                  Proceed to Checkout
                </button>
                <button
                  className="w-full mt-3 text-gray-600 hover:text-gray-800 py-2.5 rounded-lg transition-all text-base font-medium cursor-pointer"
                  onClick={handleCloseCart}
                >
                  Continue Shopping
                </button>
